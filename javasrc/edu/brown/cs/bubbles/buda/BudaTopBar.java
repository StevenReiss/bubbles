...
BudaTopBar(BudaRoot br,Element cfg,BudaBubbleArea bba,BudaOverviewBar bob)
{
   ...
   BoardColors.setColors(this,TOP_BAR_COLOR_PROP);
   ...
   addButton(bubble_menu,"Enable/Disable Shade","Enable/Disable Top Bar Shade");
...
@Override public void actionPerformed(ActionEvent e)
{
   ...
   if (cmd.equals("Save All")) {
	 ...
	 if (bt != null) buda_root.addTask(bt);
      ...
      cur_workingset.removeBubbles();
      buda_root.removeWorkingSet(cur_workingset);
      ChevronButton cb = chevron_buttons.get(cur_workingset);
      if (cb != null) {
	 remove(cb);
	 chevron_buttons.remove(cur_workingset);
       }
   ...
   else if (cmd.equals("Enable/Disable Shade")) {
      buda_root.forceShadeDown();
    }
   else if (cmd.equals("Manage Plugins")) {
...
@Override protected void paintComponent(Graphics g0)
{
   ...
   Rectangle r = getBounds();
   Color tc = BoardColors.getColor(TOP_BAR_TOP_COLOR_PROP);
   Color bc = BoardColors.getColor(TOP_BAR_BOTTOM_COLOR_PROP);
   ...
   for (BudaWorkingSetImpl ws : buda_root.getWorkingSetImpls()) {
	 ...
	 if (but == null) {
	    int rh = r.height;
	    if (rh < 12) rh = 12;
	    chevron_buttons.put(ws, new ChevronButton(ws, x1, rh/2, x2-x1, rh/2));
   ...
   if (!inws && BUDA_PROPERTIES.getBoolean(OVERVIEW_FAKE_WS_BOOL)) {
      ...
      Color ftc = BoardColors.getColor(OVERVIEW_FAKE_TOP_COLOR_PROP);
      g.setColor(ftc);
      g.fillRect(x0,0,x2-x0,r.height);
      Color fxc = BoardColors.getColor(OVERVIEW_FAKE_TEXT_COLOR_PROP);
      g.setColor(fxc);
      SwingText.drawText("Create Task...", g, new Rectangle(x0,0,x1-x0,r.height));
      if (fake_chevron_button == null && r.height > 4) {
	 fake_chevron_button = new ChevronButton(null, x1,r.height/2, x2-x1, r.height/2);
	 add(fake_chevron_button);
       }
      else if (fake_chevron_button != null) {
