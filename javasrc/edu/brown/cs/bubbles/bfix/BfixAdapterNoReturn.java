...
private static Pattern         message_pattern;

static {
   message_pattern = Pattern.compile("This method must return a result of type ([A-Za-z0-9_$.]+)");
}
...
private String getReturnType(BfixCorrector corr,BumpProblem bp)
{
   ...
   Matcher m = message_pattern.matcher(bp.getMessage());
...
private static class ReturnFixer extends BfixFixer {
   ...
   @Override protected RunnableFix findFix() {
      ...
      ReturnDoer rd = new ReturnDoer(for_corrector,for_document,for_problem,stmt,initial_time);
